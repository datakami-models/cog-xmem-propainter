...
def setup():
    ...
    return device, fix_raft, fix_flow_complete, model
    
def infer(device, fix_raft, fix_flow_complete, model, video, mask, output, resize_ratio, height, width, mask_dilation, ref_stride, neighbor_length, subvideo_length, raft_iter, mode, scale_h, scale_w, save_fps, save_frames, fp16):
    ...
    input_video = os.path.join(save_root, 'masked_in.mp4')
    output_video = os.path.join(save_root, 'inpaint_out.mp4')
    imageio.mimwrite(input_video, masked_frame_for_save, fps=fps, quality=7)
    imageio.mimwrite(output_video, comp_frames, fps=fps, quality=7)
...
if __name__ == '__main__':
    ...
    device, fix_raft, fix_flow_complete, model = setup()
    input_video, output_video = infer(device, fix_raft, fix_flow_complete, model, **args)
